...
def run_video():
    ...
    while True:
        ...
        while (cap.isOpened()):
            ...
            if ret == True:
                # total += 1
                # equal to lock.acquire() when entering 'with' block and lock.release when exiting 'with' block
                # using lock to synchronize with generate method, which is reading outputFrame
                with lock:
        ...
        for column in range(3):
            if hochregallager.behaelter_arr[row][column] is None:
                ...
                if hochregallager.grid_cell_timer_arr[row][column] == 0 and not hochregallager.grid_successfully_initialized:
                    ...
                    missing = str(hochregallager.grid_cell_timer_arr[row][column])
...
if __name__ == '__main__':
    ...
    # daemon threads are killed automatically when exiting the program
    thread.daemon = True
    thread.start()
